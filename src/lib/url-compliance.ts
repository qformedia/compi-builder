export type UrlNetwork =
  | "instagram"
  | "tiktok"
  | "youtube"
  | "pinterest"
  | "bilibili"
  | "douyin"
  | "kuaishou"
  | "xiaohongshu"
  | "other";

export type ComplianceStatus = "ok" | "fixed" | "invalid";

export interface UrlComplianceResult {
  originalUrl: string;
  fixedUrl: string;
  network: UrlNetwork;
  status: ComplianceStatus;
  issues: string[];
}

function detectNetwork(url: string): UrlNetwork {
  const lower = url.toLowerCase();
  if (lower.includes("instagram.com")) return "instagram";
  if (lower.includes("tiktok.com")) return "tiktok";
  if (lower.includes("youtube.com") || lower.includes("youtu.be")) return "youtube";
  if (lower.includes("pinterest.")) return "pinterest";
  if (lower.includes("bilibili.com")) return "bilibili";
  if (lower.includes("douyin.com") || lower.includes("iesdouyin.com")) return "douyin";
  if (lower.includes("kuaishou.com")) return "kuaishou";
  if (lower.includes("xiaohongshu.com")) return "xiaohongshu";
  return "other";
}

function fixInstagram(url: string, issues: string[]): { url: string; valid: boolean } {
  if (url.includes("?")) { url = url.split("?")[0]; issues.push("stripped query params"); }
  if (url.includes("#")) { url = url.split("#")[0]; }

  if (/\/reels\/[A-Za-z0-9_-]+/.test(url)) {
    const m = url.match(/\/reels\/([A-Za-z0-9_-]+)\/?$/);
    if (m) { url = url.replace(/\/reels\/([A-Za-z0-9_-]+)\/?$/, "/reel/$1/"); issues.push("/reels/ → /reel/"); }
  }
  if (/\/p\/[A-Za-z0-9_-]+/.test(url)) {
    url = url.replace(/\/p\/([A-Za-z0-9_-]+)\/?$/, "/reel/$1/");
    issues.push("/p/ → /reel/");
  }

  const afterDomain = url.replace(/https?:\/\/www\.instagram\.com\//, "");
  const parts = afterDomain.replace(/^\/|\/$/g, "").split("/");
  if (parts.length >= 3 && !["reel", "tv", "p", "reels", "stories"].includes(parts[0])) {
    if (["reel", "tv"].includes(parts[1]) && parts[2]) {
      url = `https://www.instagram.com/${parts[1]}/${parts[2]}/`;
      issues.push("stripped username prefix");
    }
  }

  if (/^https:\/\/www\.instagram\.com\/(reel|tv)\/[A-Za-z0-9_-]+$/.test(url)) {
    url += "/";
    issues.push("added trailing /");
  }

  const valid = /^https:\/\/www\.instagram\.com\/(reel|tv)\/[A-Za-z0-9_-]+\/$/.test(url);
  if (!valid && issues.length === 0) issues.push("non-standard URL — manual review");
  return { url, valid };
}

function fixTiktok(url: string, issues: string[]): { url: string; valid: boolean; earlyInvalid?: string } {
  if ((url.match(/http/g) || []).length > 1) {
    return { url, valid: false, earlyInvalid: "two URLs in same field — manual review" };
  }
  if (!url.startsWith("http")) { url = "https://www." + url.replace(/^\/+/, ""); issues.push("added https://www."); }
  if (url.includes("?")) { url = url.split("?")[0]; issues.push("stripped query params"); }
  if (url.includes("#")) { url = url.split("#")[0]; }

  const trailingMatch = url.match(/^(https:\/\/www\.tiktok\.com\/@[^/]+\/video\/\d+)\/$/);
  if (trailingMatch) { url = trailingMatch[1]; issues.push("removed trailing /"); }

  if (url.includes("/photo/")) {
    return { url, valid: false, earlyInvalid: "photo post — manual review" };
  }
  if (!url.includes("/video/")) {
    return { url, valid: false, earlyInvalid: "creator profile URL — manual review" };
  }

  const valid = /^https:\/\/www\.tiktok\.com\/@[^/]+\/video\/\d{15,19}$/.test(url);
  if (!valid && issues.length === 0) issues.push("non-standard URL — manual review");
  return { url, valid };
}

function fixYoutube(url: string, issues: string[]): { url: string; valid: boolean; earlyInvalid?: string } {
  if (/^https?:\/\/www\.youtu\.be\//.test(url)) {
    url = url.replace(/^https?:\/\/www\.youtu\.be\//, "https://youtu.be/");
    issues.push("stripped www. from youtu.be");
  }
  if (url.includes("youtu.be/") && url.includes("?")) { url = url.split("?")[0]; issues.push("stripped query params"); }

  const watchMatch = url.match(/^https?:\/\/(?:www\.)?youtube\.com\/watch\?v=([A-Za-z0-9_-]{11})/);
  if (watchMatch) { url = `https://youtu.be/${watchMatch[1]}`; issues.push("watch?v= → youtu.be/"); }

  if (url.includes("youtube.com/shorts/") && url.includes("?")) {
    url = url.split("?")[0]; issues.push("stripped query params from shorts");
  }
  if (/^https:\/\/youtu\.be\/[A-Za-z0-9_-]{11}\/$/.test(url)) { url = url.replace(/\/$/, ""); issues.push("removed trailing /"); }
  if (/^https:\/\/www\.youtube\.com\/shorts\/[A-Za-z0-9_-]{11}\/$/.test(url)) { url = url.replace(/\/$/, ""); issues.push("removed trailing /"); }

  if (/youtube\.com\/(@|c\/|channel\/)/.test(url)) {
    return { url, valid: false, earlyInvalid: "channel/user page — manual review" };
  }

  const valid =
    /^https:\/\/www\.youtube\.com\/shorts\/[A-Za-z0-9_-]{11}$/.test(url) ||
    /^https:\/\/youtu\.be\/[A-Za-z0-9_-]{11}$/.test(url);
  if (!valid && issues.length === 0) issues.push("non-standard URL — manual review");
  return { url, valid };
}

function fixPinterest(url: string, issues: string[]): { url: string; valid: boolean } {
  if (url.includes("?")) { url = url.split("?")[0]; issues.push("stripped query params"); }

  const pinDomain = url.match(/^(https?:\/\/)([^/]*pinterest[^/]*)(\/.*)/);
  if (pinDomain && pinDomain[2] !== "www.pinterest.es") {
    url = `https://www.pinterest.es${pinDomain[3]}`;
    issues.push("normalized domain → www.pinterest.es");
  }
  if (/^https:\/\/www\.pinterest\.es\/pin\/[^/]+$/.test(url)) { url += "/"; issues.push("added trailing /"); }

  const valid = /^https:\/\/www\.pinterest\.es\/pin\/[A-Za-z0-9_-]+\/$/.test(url);
  if (!valid && issues.length === 0) issues.push("non-standard URL — manual review");
  return { url, valid };
}

function fixBilibili(url: string, issues: string[]): { url: string; valid: boolean } {
  if (url.includes("?")) { url = url.split("?")[0]; issues.push("stripped query params"); }
  if (/^https:\/\/www\.bilibili\.com\/video\/BV[A-Za-z0-9]+$/.test(url)) { url += "/"; issues.push("added trailing /"); }

  const valid = /^https:\/\/www\.bilibili\.com\/video\/BV[A-Za-z0-9]+\/$/.test(url);
  if (!valid && issues.length === 0) issues.push("non-standard URL — manual review");
  return { url, valid };
}

function fixDouyin(url: string, issues: string[]): { url: string; valid: boolean; earlyInvalid?: string } {
  const iesMatch = url.match(/^https?:\/\/(?:www\.)?iesdouyin\.com\/share\/video\/(\d+)\/?/);
  if (iesMatch) { url = `https://www.douyin.com/video/${iesMatch[1]}`; issues.push("iesdouyin → douyin.com/video/"); }

  const modalMatch = url.match(/^https?:\/\/(?:www\.)?douyin\.com\/user\/[^?]+\?modal_id=(\d+)/);
  if (modalMatch) { url = `https://www.douyin.com/video/${modalMatch[1]}`; issues.push("extracted modal_id → douyin.com/video/"); }

  if (url.includes("v.douyin.com")) {
    return { url, valid: false, earlyInvalid: "v.douyin.com short link — requires redirect resolution; manual review" };
  }
  if (url.includes("?")) { url = url.split("?")[0]; issues.push("stripped query params"); }

  const dyTrailing = url.match(/^(https:\/\/www\.douyin\.com\/video\/\d+)\/$/);
  if (dyTrailing) { url = dyTrailing[1]; issues.push("removed trailing /"); }

  if (/^https?:\/\/(?:www\.)?douyin\.com\/user\//.test(url)) {
    return { url, valid: false, earlyInvalid: "creator profile — manual review" };
  }

  const valid = /^https:\/\/www\.douyin\.com\/video\/\d{15,}$/.test(url);
  if (!valid && issues.length === 0) issues.push("non-standard URL — manual review");
  return { url, valid };
}

export function checkUrlCompliance(inputUrl: string): UrlComplianceResult {
  const originalUrl = inputUrl.trim();
  if (!originalUrl) {
    return { originalUrl, fixedUrl: "", network: "other", status: "invalid", issues: ["empty URL"] };
  }

  let url = originalUrl;
  const network = detectNetwork(url);

  if (["kuaishou", "xiaohongshu", "other"].includes(network)) {
    return { originalUrl, fixedUrl: url, network, status: "ok", issues: [] };
  }

  const issues: string[] = [];

  // General: fix www.ww. / www.www. typo on any domain
  if (/www\.ww\./.test(url)) {
    url = url.replace(/www\.ww\./, "www.");
    issues.push("fixed www.ww. typo");
  }
  if (/www\.www\./.test(url)) {
    url = url.replace(/www\.www\./, "www.");
    issues.push("fixed www.www. typo");
  }

  let valid = false;

  switch (network) {
    case "instagram": {
      const r = fixInstagram(url, issues);
      url = r.url; valid = r.valid;
      break;
    }
    case "tiktok": {
      const r = fixTiktok(url, issues);
      url = r.url; valid = r.valid;
      if (r.earlyInvalid) return { originalUrl, fixedUrl: url, network, status: "invalid", issues: [r.earlyInvalid] };
      break;
    }
    case "youtube": {
      const r = fixYoutube(url, issues);
      url = r.url; valid = r.valid;
      if (r.earlyInvalid) return { originalUrl, fixedUrl: url, network, status: "invalid", issues: [r.earlyInvalid] };
      break;
    }
    case "pinterest": {
      const r = fixPinterest(url, issues);
      url = r.url; valid = r.valid;
      break;
    }
    case "bilibili": {
      const r = fixBilibili(url, issues);
      url = r.url; valid = r.valid;
      break;
    }
    case "douyin": {
      const r = fixDouyin(url, issues);
      url = r.url; valid = r.valid;
      if (r.earlyInvalid) return { originalUrl, fixedUrl: url, network, status: "invalid", issues: [r.earlyInvalid] };
      break;
    }
  }

  let status: ComplianceStatus;
  if (valid && url === originalUrl) status = "ok";
  else if (valid) status = "fixed";
  else status = "invalid";

  return { originalUrl, fixedUrl: url, network, status, issues };
}

export function checkAllUrls(urls: string[]): UrlComplianceResult[] {
  return urls.map(checkUrlCompliance);
}
